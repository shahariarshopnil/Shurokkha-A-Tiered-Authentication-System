...
const now = () => Date.now();
const BD = '০১২৩৪৫৬৭৮৯';
const bn = n => String(n).replace(/\d/g, d => BD[+d]);
...
const taka = n => bn(grp(n)) + ' টাকা';
const clock = () => { const d = new Date(); return bn(d.getHours()%12||12) + ':' + bn(String(d.getMinutes()).padStart(2,'0')); };
const ago = t => { const s = Math.round((now()-t)/1000); return s < 60 ? bn(s)+' সেকেন্ড আগে' : bn(Math.round(s/60))+' মিনিট আগে'; };
